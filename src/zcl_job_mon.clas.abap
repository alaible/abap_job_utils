CLASS zcl_job_mon DEFINITION PUBLIC CREATE PUBLIC INHERITING FROM zcl_job_base.
  ...
  PUBLIC SECTION.
    METHODS: constructor
        ...
        RAISING   zcx_no_entry_found zcx_job_error.
protected section.
private section.
ENDCLASS.
...
*       CATCH zcx_job_no_entry_found. " Allg. Fehlerklasse für nicht gef. Eintrag
    ELSE.
      DATA(lt_job_res) = me->read_job( iv_jobname = iv_job_src_name iv_jobcount = iv_src_job_cnt iv_status = btc_scheduled ).
      IF lines( lt_job_res ) NE 1.
        RAISE EXCEPTION TYPE zcx_job_error.
      ENDIF.
      me->ms_job_cpy_source = lt_job_res[ 1 ].
    ENDIF.
  ENDMETHOD.
ENDCLASS.
